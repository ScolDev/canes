...
export class Brk {
  ...
  execute () {
    ...
    const irqInterruptVector = this.#cpu.memory.loadWord(CPU_MEMORY_MAP.IRQ_Vector)
    const currentSP = this.#cpu.getRegister(CPU_REGISTERS.SP)

    this.#cpuALU.setFlag(CPU_FLAGS.BreakCommand)

    this.#cpu.memory.store(CPU_MEMORY_MAP.Stack + currentSP, pcl)
    this.#cpu.memory.store(CPU_MEMORY_MAP.Stack + (currentSP - 1), pch)
    this.#cpu.memory.store(CPU_MEMORY_MAP.Stack + (currentSP - 2), this.#cpu.getRegister(CPU_REGISTERS.P))
